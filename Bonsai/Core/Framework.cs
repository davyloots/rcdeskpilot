...
namespace Bonsai.Core
{
    ...
    public class Framework : IDisposable
    {
        ...
        public const int DefaultSizeWidth = 640; //768; 
        //public const int DefaultSizeWidth = 768; 
        //public const int DefaultSizeWidth = 853; 
        public const int DefaultSizeHeight = 480; //432;
        //public const int DefaultSizeHeight = 432; //432;
        //public const int DefaultSizeHeight = 600;
        ...
        private CameraBase currentCamera = null;
        ...
        /// <summary>
        /// Gets/Sets the string used for debugpurposes.
        /// </summary>
        ...
        /// <summary>
        /// Gets whether there is currently a dialog showing.
        /// </summary>
        public bool IsDialogShowing
        {
            get { return State.IsShowingDialog; }
        }
        ...
        /// <summary>
        /// Gets/Sets the camera currently in use.
        /// </summary>
        ...
        /// <summary>
        /// Calls into CreateWindow with default paramters and given icon
        /// </summary>
        public void CreateWindow(string windowTitle, Icon icon)
        {
            CreateWindow(windowTitle, icon, null, -1, -1);
        }
        /// <summary>
        ...
        public void Reset3DEnvironment()
        {
            ...
            if (DeviceLost != null)
                DeviceLost(this, System.EventArgs.Empty);
            ...
            catch (Exception e)
            {
                ...
                OnDeviceLost(null, System.EventArgs.Empty);
                if (!(e is DeviceLostException))
                {
                    throw;
                }
        ...
        public void Render3DEnvironment()
        {
            ...
            if (State.CurrentDialog != null && State.IsShowingDialog)
            {
                if (!State.IsRenderingPaused)
                {
                    // Clear the render target and the zbuffer 
                    device.Clear(ClearFlags.Target, 0x00333333, 1.0f, 0);

                    // Render the scene
                    device.BeginScene();
                    State.CurrentDialog.OnRender(elapsedTime);
            ...
            else
            {
                ...
                if (this.OnFrameMove(device, time, elapsedTime))
                    return;
                if (!State.IsRenderingPaused)
                {
                    if (this.OnFrameRender(device, time, elapsedTime))
                        return;
                }                
            }
        ...
        public void ShowSettingsDialog(bool shouldShow)
        {
            ...
            State.IsShowingDialog = shouldShow;

            if (shouldShow)
            {
                SettingsDialog dialog = PrepareSettingsDialog();
                dialog.Refresh();
                State.CurrentDialog = State.Settings;
            }
        }
        ...
        /// has not already been called, it will call CreateDevice() with the default parameters.  
        /// </summary>
        public void MainLoop(System.Windows.Forms.Form formContainer)
        {
            ...
            using (System.Windows.Forms.Control app = Window)
            {
                ...
                else
                {
                    ...
                    {
                        System.Windows.Forms.Application.Run(formContainer);
                    }
                    else
                    {
                        // Show the window
        ...
        private DeviceSettings BuildOptimalDeviceSettings(DeviceSettings settings, MatchOptions match)
        {
            ...
            // Texture Filter
            //------------------------
            optimal.TextureFilter = settings.TextureFilter;
        ...
        private DeviceSettings BuildValidDeviceSettings(EnumDeviceSettingsCombo deviceCombo, DeviceSettings settings, MatchOptions match)
        {
            ...
            // Texture filter
            //-----------------
            validSettings.TextureFilter = settings.TextureFilter;
        ...
        private void Cleanup3DEnvironment(bool releaseSettings)
        {
            ...
            if (State.Settings != null)
            {
                ...
                State.Settings.OnDestroyDevice(this, System.EventArgs.Empty);
            }
        ...
        public int SetTimer(Bonsai.Core.EventArgs.TimerCallback callbackTimer, float timeoutInSeconds)
        {
        ...
        private void OnPaint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
                ...
                (State.AreDeviceObjectsReset) && (State.IsRenderingPaused))
            {
                ...
                else if (State.CurrentDialog != null && State.IsShowingDialog)
                {
                    // Clear the render target and the zbuffer 
                    State.Device.Clear(ClearFlags.Target, 0x00003F3F, 1.0f, 0);
...
*/
        ...
        /// </summary>
        private void OnCursorChanged(object sender, System.EventArgs e)
        {
        ...
        /// </summary>
        private void OnMenuEnd(object sender, System.EventArgs e)
        {
            Pause(false, false);
        }
        ...
        internal void WindowsProcedure(ref System.Windows.Forms.Message m)
        {
            ...
            {
                State.CurrentDialog.HandleMessages(m.HWnd, (NativeMethods.WindowMessage)m.Msg, m.WParam, m.LParam);
            }
            else
            {
        ...
        private void OnDeviceLost(object sender, System.EventArgs e)
        {
            // Mark as being inside a callback
            State.IsInsideDeviceCallback = true;
        ...
        public bool OnFrameMove(Device device, double totalTime, float elapsedTime)
        {
            // Animate the lights
            foreach (LightBase light in lights)
                light.OnFrameMove(device, totalTime, elapsedTime);

            //Animate the scene by calling the app's frame move callback
            if (State.CallbackInterface != null)
            {
                State.CallbackInterface.OnFrameMove(device, totalTime, elapsedTime);
                device = State.Device;
                if (device == null)
                    return true;
            }

            // Animate the current camera
            if (currentCamera != null)
                currentCamera.OnFrameMove(device, totalTime, elapsedTime);

            return false;
        }

        public bool OnFrameRender(Device device, double totalTime, float elapsedTime)
        {
#if DEBUG
            // Clear the debugstring
            debugString = "";
#endif
            // Render the current camera
            if (currentCamera != null)
                currentCamera.OnFrameRender(device, totalTime, elapsedTime);
            
            
            // Render the lights
            foreach (LightBase light in lights)
                light.OnFrameRender(device, totalTime, elapsedTime);

            DeviceSettings settings = State.CurrentDeviceSettings;
            device.SamplerState[0].MinFilter = settings.TextureFilter;
            device.SamplerState[0].MagFilter = settings.TextureFilter;

            // Render the scene by calling the app's render callback
            if (State.CallbackInterface != null)
            {
                State.CallbackInterface.OnFrameRender(device, totalTime, elapsedTime);
                device = State.Device;
                if (device == null)
                    return true;
            }
